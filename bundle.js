...
/***/ function(module, exports, __webpack_require__) {
	...
	var onCreateGame = function onCreateGame(event) {
	  ...
	  api.create(event.target).done(ui.onSuccess).fail(ui.onError);
	};
	...
	var game = function game() {
	  ...
	  $("#R1C1, #R1C2, #R1C3, #R2C1, #R2C2, #R2C3, #R3C1, #R3C2, #R3C3").click(function () {
	    ...
	    if (turn === "O") {
	      ...
	      api.update(index, value).done(ui.onUpdate).fail(ui.onError);
	      ...
	      var _index = cell.data("index");
	      var _value = cell.val();

	      api.update(_index, _value).done(ui.onUpdate).fail(ui.onError);
